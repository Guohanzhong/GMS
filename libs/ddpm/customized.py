...
class Model4Pretrained_three(Model4Pretrained):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.requires_grad_(False)
        self.before_out3 = ResnetBlock(in_channels=self.block_in,
                                          out_channels=self.block_in,
                                          temb_channels=self.temb_ch,
                                          dropout=self.dropout)
        self.norm_out3 = Normalize(self.block_in)
        self.conv_out3 = torch.nn.Conv2d(self.block_in,
                                         self.head_out_ch,
                                         kernel_size=3,
                                         stride=1,
                                         padding=1)
    ...
    def forward(self, x, t):
        ...
        out = self.conv_out(nonlinearity(self.norm_out(h)))
        out2 = self.conv_out2(nonlinearity(self.norm_out2(self.before_out(h, temb))))
        out3 = self.conv_out3(nonlinearity(self.norm_out3(self.before_out3(h, temb))))
