...
#ns = NoiseScheduleVP('discrete', betas=betas)
#print(ns.marginal_lambda(torch.tensor(0.7)))
...
class DPM_Solver:
    ...
    def model_fn(self, x, t):
        ...
        """
        #t = t.to(self.model_fn.device)
        #x = x.to(self.model_fn.device)
        ...
        if self.algorithm_type == "dpmsolver++":
            ...
            if model_s is None:
                try:
                    ...
                    logging.info(s)
                    model_s = self.model_fn(x, x.new_ones([x.shape[0], ], dtype=torch.long)  * s)
            x_t = (
                sigma_t / sigma_s * x
            ...
            phi_1 = torch.expm1(h)
            s = x.new_ones([x.shape[0], ], dtype=torch.long)  * s
            if model_s is None:
                try:
                    model_s = self.model_fn(x, s)
                except:
    ...
    def singlestep_dpm_solver_second_update(self, x, s, t, r1=0.5, model_s=None, return_intermediate=False, solver_type='dpmsolver'):
        ...
        if self.algorithm_type == "dpmsolver++":
            ...
            if model_s is None:
                try:
                    model_s = self.model_fn(x, s*1000)
                except:
                    logging.info(s*1000)
                    model_s = self.model_fn(x, x.new_ones([x.shape[0], ], dtype=torch.long)  * s)
            x_s1 = (
        ...
        if self.algorithm_type == "dpmsolver++":
            ...
            if model_s1 is None:
                ...
                try:
                    ...
                    logging.info(s1)
                    model_s1 = self.model_fn(x_s1, x.new_ones([x.shape[0], ], dtype=torch.long)  * s1)
                #model_s1 = self.model_fn(x_s1, s1)
            x_s2 = (
                (sigma_s2 / sigma_s) * x
            ...
            try:
                ...
                #logging.info(s2)
                model_s2 = self.model_fn(x_s2, x.new_ones([x.shape[0], ], dtype=torch.long)  * s2)
            if solver_type == 'dpmsolver':
                x_t = (
                    (sigma_t / sigma_s) * x
                    - (alpha_t * phi_1) * model_s
            ...
            if model_s is None:
                ...
                #s = s.to(model_s.device)
                #logging.info(s)
                model_s = self.model_fn(x, s*1000).to(x.device)
            if model_s1 is None:
                x_s1 = (
                    (torch.exp(log_alpha_s1 - log_alpha_s)) * x
                    - (sigma_s1 * phi_11) * model_s
                )
                model_s1 = self.model_fn(x_s1, s1*1000)
            x_s2 = (
                (torch.exp(log_alpha_s2 - log_alpha_s)) * x
                - (sigma_s2 * phi_12) * model_s
                - r2 / r1 * (sigma_s2 * phi_22) * (model_s1 - model_s)
            )
            model_s2 = self.model_fn(x_s2, s2*1000)
            if solver_type == 'dpmsolver':
                x_t = (
                    (torch.exp(log_alpha_t - log_alpha_s)) * x
                    - (sigma_t * phi_1) * model_s
...
if  __name__ == "__main__":
    betas = torch.linspace(1e-4, 0.02, 1000).double()
    ns = NoiseScheduleVP('discrete', betas=betas)
    print(ns.marginal_lambda(torch.tensor(1.0)))
    alphas = 1. - betas
    alphas_bar = torch.cumprod(alphas, dim=0)
    log_alphas = 0.5 * torch.log(1 - betas).cumsum(dim=0)
    log_alpha_array = log_alphas.reshape((1, -1,))
    one_minus_alphas_bar = (1.- alphas_bar)
    sqrt_alphas_bar =  torch.sqrt(alphas_bar)
    x_t = torch.tensor([1.])
    t = x_t.new_ones([x_t.shape[0], ], dtype=torch.long)  * 999
    a_t  = extract(sqrt_alphas_bar, t, x_t.shape)
    sigma_t = torch.sqrt(extract(one_minus_alphas_bar, t, x_t.shape))
    print(torch.log(a_t)-torch.log(sigma_t))